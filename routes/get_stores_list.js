...
module.exports = function (req,res){
  ...
  dbconnect.get_followed_merchants_count(buyer_id, function(followed_merchant_error, followed_merchants) {
    if (followed_merchant_error) {
        var error_key = "get_followed_merchants_count_"+Date.now();
        logger.log({
            level: 'error',
            time_stamp: Date.now(),
            message: followed_merchant_error.message,
            error_key: error_key
        });
        ...
        if (followed_merchants[0].count >= 3) {
                        ...
                        if (err) {
                            ...
                            var grouped_by_store_id = _.groupBy(results.map(result_obj => {
                                return {
                                    ...
                                    payment_url: "http://chernas.mobyuat.asia/mobilepost_app.html?post_id=" + result_obj.ig_post_id,
                                }
                            }), "store_id");
                            ...
                            res.json({
                                merchants_followed: followed_merchants[0].count,
                                minimum_merchants_required_to_follow: 3,
            ...
            dbconnect.get_all_stores_list(buyer_id,function(error, results ){
                if (error) {
